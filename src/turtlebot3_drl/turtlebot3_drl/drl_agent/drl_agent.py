...
class DrlAgent(Node):
    def __init__(self, training, algorithm, load_session="", load_episode=0, real_robot=0, resume=False):
    ...
    def process(self):
        ...
        try:
            while True:
                ...
                if self.total_steps > self.observe_steps and self.episode % CHECKPOINT_INTERVAL == 0:
            ...
            # Ensure simulation is paused before exiting
            util.pause_simulation(self, self.real_robot)
    ...
    def save_checkpoint(self):
        checkpoint_path = os.path.join(self.sm.session_dir, 'checkpoints')
        ...
        checkpoint = {
            ...
            'graph_data': self.graph.graphdata
        }
    
        torch.save(checkpoint, os.path.join(checkpoint_path, f'checkpoint_{self.episode}.pt'))
        print(f"Checkpoint saved at episode {self.episode}")
    
    def load_checkpoint(self):
        checkpoint_path = os.path.join(self.sm.session_dir, 'checkpoints')
        if not os.path.exists(checkpoint_path):
            print("No checkpoints folder found. Starting from scratch.")
            return
        ...
        self.model.load_optimizer_state_dict(checkpoint['optimizer_state_dict'])
        self.replay_buffer.buffer = checkpoint['replay_buffer']
        self.graph.graphdata = checkpoint['graph_data']
        print(f"Resumed training from checkpoint at episode {self.episode}")
...
def main(args=sys.argv[1:]):
    ...
    if resume:
        args.remove('--resume')
    drl_agent = DrlAgent(*args, resume=resume)
