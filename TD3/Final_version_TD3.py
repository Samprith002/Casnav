            ...
            for j in range(-self.observation_radius, self.observation_radius + 1):
                ...
                if 0 <= xi < self.grid_size and 0 <= yj < self.grid_size:
                    if (xi, yj) == self.goal_position:
                        obs[i + self.observation_radius, j + self.observation_radius] = 1  # Goal position marked as 1
                    elif any(np.linalg.norm(np.array([xi, yj]) - np.array(human_pos)) < 1.0 for human_pos in self.human_positions):
                        obs[i + self.observation_radius, j + self.observation_radius] = -1  # Human positions marked as -1
                    elif [xi, yj] == self.robot_position:
                        obs[i + self.observation_radius, j + self.observation_radius] = 2  # Robot position marked as 2
...
def main():
    ...
    for episode in range(start_episode, num_episodes):
        ...
        if episode % 10 == 0:
            ...
            save_checkpoint(checkpoint, os.path.join(checkpoint_dir, "checkpoint_latest.pth"))
        ...
        if episode_reward > best_reward:
            ...
            save_checkpoint(best_checkpoint, os.path.join(checkpoint_dir, "best_model.pth"))
